...
import { HiOutlineUser, HiOutlineShoppingBag, HiOutlineLogout, HiCog, HiClipboardList } from "react-icons/hi";
...
const Navbar = () => {
  ...
  return (
    <>
      <nav className="container mx-auto grid grid-cols-3 items-center gap-2 py-3 px-3 md:px-6 md:py-4">
        {/* Logo - left */}
        <Link to="/" className="flex items-center justify-self-start shrink-0 min-w-0">
          <img
            src={tallBoyLogo}
        ...
        {/* Options - centered, contained in the nav bar on every screen size */}
        <div className="flex items-center justify-center space-x-4 md:space-x-8 overflow-x-auto min-w-0 max-w-full">
          {NAV_LINKS.map((link) => (
            <Link
              key={link.to}
              to={link.to}
              className="text-gray-700 hover:text-gray-900 text-xs md:text-sm font-medium uppercase tracking-wide transition-colors whitespace-nowrap"
            >
              {link.label}
            </Link>
          ))}
        ...
        <div className="relative flex items-center justify-end gap-2 md:gap-4 shrink-0 min-w-0">
          {/* Mobile-only user / login icon */}
          <button
          ...
          {/* Mobile-only cart with red corner badge */}
          <CartButton onClick={toggleCartDrawer} className="md:hidden" />

          {/* Desktop icons: user, cart, search */}
          ...
          {/* Shared user dropdown - anchored to the right of the actions area */}
          {userMenuOpen && (
